    ...
    App::new()
        .add_plugins(DefaultPlugins.set(WindowPlugin {
            primary_window: Some(Window {
                title: "Hanabi VFX - Steam".to_string(),
...
fn setup(mut commands: Commands) {
    ...
    commands.spawn((
        Text::new(
            "Steam Effect\n\n\
            Press SPACE - Spawn steam puff at cursor\n\
            Press ESC - Exit"
        ),
        Node {
...
fn spawn_effect(
    ...
    if keyboard.just_pressed(KeyCode::Space) {
        info!("Spawning steam effect at {:?}", cursor_world_pos);
        commands.trigger(VfxEvent::steam(cursor_world_pos));
    }
}
