...
/// Marker for the animated fill bar demo
#[derive(Component)]
struct AnimatedFill;
...
fn setup(
    ...
    let mat_animated = materials.add(ChromaticAberrationV2Material {
    ...
    // Center: half fill — green tint at 50%
    let mat_half = materials.add(ChromaticAberrationV2Material {
        texture: texture.clone(),
        amount: 0.05,
        alpha: 1.0,
        fill: 0.5,
    ...
    let mat_full = materials.add(ChromaticAberrationV2Material {
        texture: texture.clone(),
        amount: 0.06,
        alpha: 1.0,
        fill: 1.0,
    ...
    commands.spawn((
        Text::new(
            "Chromatic Aberration V2 — Fill Bar\n\
...
/// Animates the fill percentage on the left mesh back and forth
fn animate_fill(
    time: Res<Time>,
    query: Query<&MeshMaterial2d<ChromaticAberrationV2Material>, With<AnimatedFill>>,
    mut materials: ResMut<Assets<ChromaticAberrationV2Material>>,
) {
    for handle in &query {
        if let Some(mat) = materials.get_mut(handle.id()) {
            // Oscillate fill between 0.0 and 1.0
